...
export async function settleAdCampaignFromChargeSuccess(data) {
  ...
  try {
    ...
    const dup = await client.query(
      `SELECT 1 FROM ad_paystack_settlements WHERE paystack_reference = $1 FOR UPDATE`,
      [reference],
    );
    if (dup.rowCount > 0) {
    ...
    await client.query(
      `UPDATE ad_campaigns SET status = CASE WHEN start_at <= NOW() THEN 'ACTIVE'::ad_status ELSE 'SCHEDULED'::ad_status END, amount_paid = $2, paystack_reference = COALESCE(paystack_reference, $3), updated_at = NOW() WHERE id = $1`,
