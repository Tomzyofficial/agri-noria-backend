...
export async function getActiveSponsoredCampaigns(country) {
  const { rows } = await pool.query(
    `SELECT ls.id, ls.listing_name, ac.start_at, ac.end_at, NOW(), ls.price, ls.image, ls.description,
              cu.currency, cu.country_code, ls.account_id, ac.id AS campaign_id, ac.placement, ac.target_id 
       FROM listings ls 
      JOIN country_utils cu ON cu.vendor_id = ls.account_id
      JOIN ad_campaigns ac ON ac.target_id = ls.id
       WHERE ac.status = 'ACTIVE'::ad_status AND ac.placement = 'SPONSORED_PRODUCT'::ad_placement AND ac.start_at <= NOW() AND end_at >= NOW() AND cu.country_code = $1`,
    [country],
  );
  return rows || null;
}
