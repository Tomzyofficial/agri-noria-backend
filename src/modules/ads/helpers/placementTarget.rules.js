/* export function assertPlacementTarget(
  ...
  if (placement === "FEATURED_VENDOR") {
    if (targetType !== "VENDOR") {
      return {
        ok: false,
        message: "FEATURED_VENDOR campaigns must target VENDOR",
      };
    }
    if (targetId !== vendorId) {
      return {
        ok: false,
        message: "FEATURED_VENDOR target must be your own vendor account",
      };
    }
    return { ok: true };
  }
  ...
  if (placement === "SPONSORED_PRODUCT" || placement === "SEARCH_BOOST") {
    if (targetType !== "PRODUCT") {
      return {
        ok: false,
        message: `${placement} campaigns must target PRODUCT (listing)`,
      };
    }
    return { ok: true };
  }
