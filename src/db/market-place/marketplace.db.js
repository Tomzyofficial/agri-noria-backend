...
export async function getAllMarketplaceProducts(country) {
  try {
    const { rows } = await pool.query(
      `SELECT ls.id, ls.image, ls.listing_name, ls.price, ls.description, cu.currency, cu.country_code FROM listings ls LEFT JOIN country_utils cu ON ls.account_id = cu.vendor_id WHERE cu.country_code = $1 AND ls.category IN ('farm_produce', 'equipment', 'food_items') AND status = 'active' ORDER BY ls.id DESC`,
      [country],
    );
    return rows;
  } catch (error) {
...
export async function getSingleMarketplaceProduct(id) {
  try {
    ...
    const { rows } = await pool.query(
      `SELECT ls.id AS listing_id, ls.*, v.fname, v.lname, v.phone, v.profile_image_url, v.is_verified, cu.country_code, cu.currency, cu.country_name, cu.state_name
