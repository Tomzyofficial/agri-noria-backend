...
SELECT 'Maize', 180.50, 'Noria Northern Hub (Kano)', 'available'
WHERE NOT EXISTS (SELECT 1 FROM warehouse_stocks);

INSERT INTO warehouse_stocks (commodity, quantity_tons, warehouse_name, status)
SELECT 'Rice', 145.20, 'Noria Western Depot (Ibadan)', 'available'
WHERE NOT EXISTS (SELECT 1 FROM warehouse_stocks WHERE commodity = 'Rice');

INSERT INTO warehouse_stocks (commodity, quantity_tons, warehouse_name, status)
SELECT 'Soybean', 124.30, 'Noria Central Silo (Abuja)', 'available'
