...
export const aggregatorController = {
  ...
  createBuyerRegistration: async (req, res) => {
    try {
      ...
      await aggregatorDb.updateAgreementStatus(agreement.id, "pending", {
        payment_token,
      });
      ...
      await aggregatorDb.updateAgreementStatus(agreement.id, "sent", {
  ...
  uploadSignedAgreement: async (req, res) => {
    try {
      ...
      const updated = await aggregatorDb.updateAgreementStatus(
        agreement_id,
        "signed",
        { signed_pdf_url },
      );
      ...
      if (
        ...
        return res
          ...
          .json({
            success: false,
            error:
              "Agreement must be signed before payment for pre-harvest crops.",
          });
      }
  ...
  getAgreements: async (req, res) => {
    try {
      ...
      const agreements = await aggregatorDb.getAgreementsByAggregator(
        payload.id,
      );
    ...
    try {
      ...
      if (role !== "finance" && role !== "super admin") {
        return res
          ...
          .json({
            success: false,
            error: "Only Finance role can release escrow",
          });
      }
      ...
      if (!agreement || agreement.payment_status !== "escrow") {
        return res
          .status(400)
          .json({ success: false, error: "Agreement not in escrow status" });
      }
    ...
    try {
      ...
      if (role !== "finance" && role !== "super admin") {
        return res
          ...
          .json({
            success: false,
            error: "Only Finance and Super Admin can view finance wallets",
          });
      }
    ...
    try {
      ...
      if (role !== "finance" && role !== "super admin") {
        return res
          ...
          .json({
            success: false,
            error: "Only Finance role can access finance wallets",
          });
      }
  ...
  updateSettings: async (req, res) => {
    try {
      ...
      const updatedProfile = await aggregatorDb.updateProfile(
        payload.id,
        req.body,
      );
    ...
    try {
      ...
      if (agreement.status === "accepted" || agreement.status === "stamped") {
        return res
          ...
          .json({
            success: false,
            error: "Agreement has already been accepted",
          });
      }
...
function drawPDFDocument(doc, agreement) {
  const product =
    typeof agreement.product_details === "string"
  ...
  // === PARTIES SECTION ===
  doc.fontSize(12).font("Helvetica-Bold").fillColor("#1a5c2e").text("PARTIES");
  doc
    .strokeColor("#dddddd")
    .lineWidth(0.5)
    .moveTo(50, doc.y)
  ...
  if (agreement.aggregator_company)
    doc.text(`  Company: ${agreement.aggregator_company}`);
  const aggName = [agreement.aggregator_fname, agreement.aggregator_lname]
    .filter(Boolean)
    .join(" ");
  if (aggName) doc.text(`  Representative: ${aggName}`);
  if (agreement.aggregator_email)
    doc.text(`  Email: ${agreement.aggregator_email}`);
  doc.moveDown(0.5);

  // Buyer
  doc.fontSize(10).font("Helvetica-Bold").fillColor("#333333").text("Buyer:");
  doc.fontSize(10).font("Helvetica").fillColor("#555555");
  if (agreement.buyer_name) doc.text(`  Name: ${agreement.buyer_name}`);
  if (agreement.buyer_company)
    doc.text(`  Company: ${agreement.buyer_company}`);
  ...
  if (agreement.buyer_address)
    doc.text(`  Address: ${agreement.buyer_address}`);
  doc.moveDown(1);
  ...
  if (product.quantity)
    doc.text(`  Quantity: ${product.quantity} ${product.unit || "units"}`);
  ...
  if (product.quality_grade)
    doc.text(`  Quality Grade: ${product.quality_grade}`);
  if (product.delivery_date)
    doc.text(`  Expected Delivery: ${formatDate(product.delivery_date)}`);
  ...
  doc
    ...
    .fillColor("#333333")
    .text(
      `  Total Procurement Value: ${formatCurrency(agreement.financing_amount)}`,
    );
  doc.moveDown(0.5);
  doc
    .fontSize(10)
    ...
    .fillColor("#555555")
    .text(
      `  Type: ${agreement.is_pre_harvest ? "Pre-Harvest (Scenario A)" : "Post-Harvest (Scenario B)"}`,
    );
  doc.moveDown(1);
  ...
  // Buyer signature
  doc
    .text("____________________________", 320, sigY)
