...
storage_facilities.create = async (req, res) => {
   ...
   try {
      ...
      const storage_img = await saveFileToCloudinary(storage_image, "marketplace_storage", "image");
      if (!storage_img) {
...
storage_facilities.editStorage = async (req, res) => {
   try {
      ...
      if (storage_image_req && typeof storage_image_req === "object") {
         try {
            storage_image = await saveFileToCloudinary(storage_image_req, "marketplace_storage", "image");
         } catch (uploadError) {
