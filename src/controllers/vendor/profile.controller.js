...
profileController.uploadProfileImage = async (req, res) => {
   try {
      ...
      const imageUrl = await saveFileToCloudinary(req.file, "vendor_profiles", "image");
      if (!imageUrl) {
   ...
   try {
      ...
      if (req.files) {
         ...
         if (req.files["id_front_url"] && req.files["id_front_url"][0]) {
            ...
            id_front_url = await saveFileToCloudinary(file, "vendor_documents", "image");
         }
         ...
         if (req.files["id_back_url"] && req.files["id_back_url"][0]) {
            ...
            id_back_url = await saveFileToCloudinary(file, "vendor_documents", "image");
         }
         ...
         if (req.files["license_url"] && req.files["license_url"][0]) {
            ...
            license_url = await saveFileToCloudinary(file, "vendor_documents", "image");
         }
      }
