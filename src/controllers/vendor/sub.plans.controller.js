...
subPlansController.getCurrent = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ success: true, subscription: subscription || null });
  } catch {
    return res
      .status(500)
  ...
  try {
    ...
    if (!vendorId || !planId || !amount || !email || !planName) {
      return res.status(400).json({
        success: false,
        error:
          "Missing required fields: vendorId, planId, amount, email, and planName are required",
      });
    }
    if (vendorId !== payload.id) {
      return res
        ...
        .json({
          success: false,
          error: "Vendor ID does not match authenticated user",
        });
    }
    ...
    const accountTypeRoutes = {
      ...
      storage_facility: "/marketplace/sub-store/",
...
subPlansController.verifyPayment = async (req, res) => {
  try {
    ...
    if (!recentransaction) {
      return res
        .status(404)
        .json({ success: false, error: "No recent record transaction found" });
    }
    ...
    const subscription = await upsertVendorSubscription({
      ...
      paystack_authorization_code:
        verifyRes.data.authorization.authorization_code,
  ...
  try {
    ...
    if (!currentSubscription.paystack_subscription_code) {
      return res
        ...
        .json({
          success: false,
          error: "Subscription cannot be cancelled - no Paystack code",
        });
    }
    ...
    LOG("CANCEL_SUBSCRIPTION_CONTROLLER_ERROR", {
      message: error.message,
      stack: error.stack,
    });
    return res.status(500).json({
...
subPlansController.downgrade = async (req, res) => {
  try {
    ...
    if (!targetPlanId) {
      return res
        .status(400)
        .json({ success: false, error: "Target plan ID is required" });
    }
    ...
    if (targetPlanResult.rowCount === 0) {
      return res
        .status(404)
        .json({ success: false, error: "Target plan not found" });
    }
    ...
    if (!currentSubscription.paystack_subscription_code) {
      return res
        ...
        .json({
          success: false,
          error: "Cannot downgrade - no Paystack subscription code",
        });
    }
    ...
    LOG("DOWNGRADE_SUBSCRIPTION_CONTROLLER_ERROR", {
      message: error.message,
      stack: error.stack,
    });
    return res.status(500).json({
...
subPlansController.getHistory = async (req, res) => {
  try {
    ...
    LOG("GET_SUBSCRIPTION_HISTORY_ERROR", {
      message: error.message,
      stack: error.stack,
    });
    return res.status(500).json({
      ...
      error:
        error.message ||
        "An error occurred while fetching subscription history",
    });
  }
};
