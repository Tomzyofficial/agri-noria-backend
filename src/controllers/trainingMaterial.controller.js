...
trainingMaterialController.uploadTrainingMaterial = async (req, res) => {
   ...
   try {
      ...
      if (!file) {
         return res.status(400).json({ success: false, error: "No file uploaded. Ensure 'file' field is included." });
      }
      ...
      const uploadVideo = await saveFileToCloudinary(file, "training_materials", resourceType);
...
trainingMaterialController.deleteTrainingMaterial = async (req, res) => {
   ...
   try {
      ...
      if (result.success) {
         ...
         res.status(400).json({ success: false, error: result.error || "Failed to delete material." });
      }
   } catch (error) {
...
trainingMaterialController.getTrainingMaterialsForVendor = async (req, res) => {
   ...
   try {
      ...
      res.status(500).json({ success: false, error: "Failed to fetch training materials." });
   }
};
