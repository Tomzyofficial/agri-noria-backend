...
import {
  ...
  verifyCartToken,
  deleteCartCookie,
} from "../../sessions/cart.cookie.session.js";
...
buyerCartController.sync = async (req, res) => {
  ...
  if (!buyerId) {
    return res
      .status(400)
      .json({ success: false, error: "Missing required body" });
  }
...
buyerCartController.set = async (req, res) => {
  try {
    ...
    if (!Array.isArray(cart)) {
      return res
        .status(400)
        .json({ success: false, error: "Cart must be an array" });
    }
...
// buyerCartController.delete = async (req, res) => {
//   try {
//     try {
//       const deleteCart = await deleteCartCookie(res);
//       return res.status(200).json({ success: true, deleteCart });
//     } catch (error) {
//       console.error("Error occurred with deleting cart from backend", error);
